...
class PlayerActivity : AppCompatActivity() {
    ...
    private var lastRequestUserAgent: String? = null
    private var lastRequestReferer: String? = null
    private var lastRequestOrigin: String? = null
    ...
    private val tvRemote by lazy {
        com.animatv.player.tv.TvPlayerRemote(
            tvHost
        )
    }
    ...
    override fun attachBaseContext(
        ...
        val lang =
            LocaleHelper.getLanguageCode(base)
    ...
    private fun bindingListener() {
        ...
        bindingRoot.btnMiniChannelToggle
            .setOnClickListener {
                toggleMiniChannelPanel()
            }
    ...
    private fun lockControl(
        ...
        bindingControl.btnSpeed.visibility =
            visibility

        bindingControl.btnSleep.visibility =
            visibility
    ...
    private fun switchLiveOrVideo(
        ...
        if (
            player?.isCurrentMediaItemSeekable == false
        ) {
            visibility = View.GONE
        }
    ...
    private fun buildClearKeyLicenseJson(
        license: String
    ): ByteArray? {
        ...
        try {
            ...
            Log.d(
                "DRM_DEBUG",
                "ClearKey JSON berhasil dibuat ($count keys)"
            )
            ...
            Log.e(
                "DRM_DEBUG",
                "buildClearKeyLicenseJson error",
                e
            )
    ...
    private fun playChannel(
        ...
        // Simpan header request terakhir
        ...
        Log.d(
            "DRM_DEBUG",
            "channel.drmName    = ${current?.drmName}"
        )

        Log.d(
            "DRM_DEBUG",
            "channel.streamType = ${current?.streamType}"
        )

        Log.d(
            "DRM_DEBUG",
            "resolved drmLicense = ${if (drmLicense.isNullOrBlank()) "NONE" else "PRESENT"}"
        )
        ...
        Log.d(
            ...
            "Channel='${current?.name}' " +
                    "format=$mimeType " +
                    "(fromField=${current?.streamType}) " +
                    "fallback=$isFormatFallbackAttempt"
        )
        ...
        val isClearKey =
            drmName.contains("clearkey") ||
                    drmName.contains("clear_key") ||
                    drmName == "ck"


        val isWidevine =
            drmName.contains("widevine") ||
                    drmName == "wv"
        ...
        if (
            ...
            try {
                ...
                if (looksLikeUrl) {
                    ...
                    Log.d(
                        "DRM_DEBUG",
                        "ClearKey license URL configured"
                    )

                } else {

                    val licenseBytes =
                        buildClearKeyLicenseJson(
                            rawLicense
                        )
                    ...
                    if (licenseBytes == null) {
                        ...
                        Log.e(
                            "DRM_DEBUG",
                            "ClearKey parse FAILED"
                        )

                        Toast.makeText(
                            applicationContext,
                            "ClearKey license tidak valid",
                            Toast.LENGTH_LONG
                        ).show()
                ...
                Log.e(
                    "DRM_DEBUG",
                    "ClearKey build error",
                    e
                )

                Toast.makeText(
                    applicationContext,
                    "ClearKey gagal inisialisasi: ${e.message}",
                    Toast.LENGTH_LONG
                ).show()
            }
        ...
        // --------------------------------------------------------
            ...
            Log.e(
                "DRM_DEBUG",
                "DRM channel ($drmName) but license NOT FOUND!"
            )

            Toast.makeText(
                applicationContext,
                "DRM license tidak ditemukan (drmName='$drmName')",
                Toast.LENGTH_LONG
            ).show()
        }
            ...
            if (httpError != null) {
                ...
                val httpLog =
                    buildString {
                        ...
                        append(
                            "url=$currentCleanStreamUrl\n"
                        )

                        append(
                            "userAgent=$lastRequestUserAgent\n"
                        )

                        append(
                            "referer=$lastRequestReferer\n"
                        )

                        append(
                            "origin=$lastRequestOrigin\n"
                        )

                        append(
                            "responseHeaders=${httpError.headerFields}\n"
                        )
                    }
    ...
    override fun onKeyUp(
        ...
        if (
            player?.isCurrentMediaItemLive == false
        ) {
